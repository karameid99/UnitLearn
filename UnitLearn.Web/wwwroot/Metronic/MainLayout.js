...
$(document).on("click", ".ConfirmNonAjax", function () {
    $("#ConfirmNonAjax").modal("show");
    $("#ConfirmNonAjax .btn-danger").attr("href", $(this).attr("href"));
    $("#ConfirmNonAjax .btn-danger").attr("tname", $(this).attr("tname"));
    return false;
});

$("#ConfirmNonAjax .btn-danger").click(function () {
    var url = $(this).attr("href");
    $.ajax({
        url: url,
        success: function (json) {
            ShowMessage("تمت عملية الحذف بنجاح");
            location.reload();
        }
    });
    $("#ConfirmNonAjax").modal("hide");
    return false;
});

//$("#ConfirmNonAjax .btn-danger").click(function () {
//    var url = $(this).attr("href");
//    var urlAction = url.substring(0, url.lastIndexOf("/"));
